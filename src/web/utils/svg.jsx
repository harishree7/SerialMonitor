import Vec2 from "./vec2.jsx";
export default class SVGReader {
  constructor() {
    this.boundarys = {};
    this.style = {};
    this.tolerance = 0.1;
    this.attrMapping = new SVGAttributeMapping();
    this.tagMapping = new SVGTagMapping();
  ...
  parse(svgstring, config) {
    this.tolerance_squared = Math.pow(this.tolerance, 2);
    // parse xml
    var svgRootElement;
    if (window.DOMParser) {
      var parser = new DOMParser();
      svgRootElement = parser.parseFromString(svgstring, "text/xml")
        .documentElement;
    } else {
  ...
  parseChildren(domNode, parentNode) {
    var childNodes = [];
    for (var i = 0; i < domNode.childNodes.length; i++) {
      ...
      if (tag.childNodes) {
        if (tag.tagName) {
          ...
          if (tag.attributes) {
            for (var j = 0; j < tag.attributes.length; j++) {
              ...
              if (
                attr.nodeName &&
                attr.nodeValue &&
                this.attrMapping[attr.nodeName]
              ) {
                this.attrMapping[attr.nodeName](this, node, attr.nodeValue);
              }
            }
          }
          ...
          if (this.tagMapping[tag.tagName]) {
            //if (node.stroke[0] == 255 && node.stroke[1] == 0 && node.stroke[2] == 0) {
            this.tagMapping[tag.tagName](this, tag, node);
            //}
          }
...
export class SVGAttributeMapping {
  constructor() {
...
export class SVGTagMapping {
  ...
  image(parser, tag, node) {
    // not supported
    // has transform and style attributes
  }

  defs(parser, tag, node) {
    // not supported
    // http://www.w3.org/TR/SVG11/struct.html#Head
    // has transform and style attributes
  }

  style(parser, tag, node) {
    ...
    //     var kv = segs[i].split(":")
    //     var k = kv[0].strip()
    //     if (this.attrMapping[k]) {
    //       var v = kv[1].strip()
    //       this.attrMapping[k].call(v, defs, st)
    //     }
    //   }
    // }
  }
