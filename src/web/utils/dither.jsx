export default class Dither {
  ...
  BayerDithering(inPixels, width, height, palette) {
    ...
    for (var j = 0; j < height; j++) {
      ...
      for (var i = 0; i < width; i++) {
        ...
        r = this.colorClamp(inPixels[offset++] + threshold);
        g = this.colorClamp(inPixels[offset++] + threshold);
        b = this.colorClamp(inPixels[offset++] + threshold);

        index = this.getClosestPaletteColorIndex(
          r,
          g,
          b,
          palette,
  ...
  ClosestDithering(inPixels, width, height, palette) {
    var offset = 0;
    var indexedOffset = 0;
    var r, g, b;
    var index;
    ...
    for (var i = 0; i < numPixels; i++) {
      ...
      indexedPixels[i] = this.getClosestPaletteColorIndex(
        r,
        g,
        b,
        palette,
  ...
  FloydSteinberg(inPixels, width, height, palette) {
    var paletteLength = palette.length;
    var offset = 0;
    var indexedOffset = 0;
    var r, g, b;
    ...
    for (var j = 0; j < height; j++) {
      for (var i = 0; i < width; i++) {
        r = this.colorClamp(inPixels[offset++]);
        g = this.colorClamp(inPixels[offset++]);
        b = this.colorClamp(inPixels[offset++]);

        var colorIndex = this.getClosestPaletteColorIndex(
          r,
          g,
          b,
          palette,
